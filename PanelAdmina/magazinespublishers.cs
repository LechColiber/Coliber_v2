...
{
    public partial class MagazinesPublishers : Form
    {
        ...
        public MagazinesPublishers(Connection NewConnection, string Caption, string SelectedTable)
        {
            InitializeComponent();
        ...
        private void setControlsText()
        {
            ...
            mapping.Add(label12,"publisher_occurs");
            ...
            if (AddButton.Text != _T("save"))
            {
                try
                {
                    ...
                    Command.CommandText = "SELECT COUNT(*) " +
                                          ...
                                          "WHERE id_wydawcy = ?; ";
                    Command.Parameters.AddWithValue("Pub", Int32.Parse(dataGridView1["ID wydawcy", e.RowIndex].Value.ToString()));
        ...
        private void ShowBooksButton_Click(object sender, EventArgs e)
        {
            ...
            {
                ShowList ShowListForm = new ShowList(GetListWherePublisherOccured(Int32.Parse(dataGridView1["ID wydawcy", dataGridView1.CurrentRow.Index].Value.ToString())), dataGridView1["Nazwa", dataGridView1.CurrentRow.Index].Value.ToString());
        ...
        private List<string> GetListWherePublisherOccured(int IDPublisher)
        {
            try
            {
                OdbcCommand Command = new OdbcCommand();
                Command.Connection = NewConnection.AppConnection;
                Command.CommandText = "SELECT LTRIM(RTRIM(tytul)) AS tytul " +
                                      "FROM " + this.SelectedTable + " " +
                                      "WHERE id_wydawcy = ? " +
                                      "ORDER BY tytul; ";
        ...
        private void AddButton_Click(object sender, EventArgs e)
        {
            ...
            {
                EditPublisherForm EPF = new EditPublisherForm(NewConnection, "", "", "", "", "", "", ReturnIDRodzaj(RodzajDataTable), 'c', -1, 1);
                EPF.ShowDialog();
                SelectFromDb(ReturnIDRodzaj(RodzajDataTable));
            }
        }
        ...
        private void MergeButton_Click(object sender, EventArgs e)
        {
            ...
            if (MergeButton.Text == _T("cancel"))
            {
                ...
                if (dataGridView1.Rows.Count > 0 && dataGridView1.CurrentRow.Index > 0)
                {
                    ...
                    dataGridView1["Nazwa", dataGridView1.Rows.Count - 1].Selected = true;
                }
            }
            else if (MergeButton.Text == _T("merge") && dataGridView1.Rows.Count > 0)
            {
                try
                {
                    OdbcCommand Command = new OdbcCommand();
                    Command.Connection = NewConnection.AppConnection;
                    Command.CommandText = "SELECT id_wydawcy AS [id], LTRIM(RTRIM(nazwa_w)) AS [Nazwa] FROM " + this.Table + " " +
                                          "LEFT JOIN dbo.rodzaje ON dbo.rodzaje.id_rodzaj = " + this.Table + ".id_rodzaj " +
                                          "WHERE id_wydawcy != ? ORDER BY nazwa_w; ";
                    Command.Parameters.AddWithValue("@IDPublisher", Int32.Parse(dataGridView1["ID wydawcy", dataGridView1.CurrentRow.Index].Value.ToString()));
                    ...
                    if (ChSForm.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                    {
                        ...
                        if (MessageBox.Show(_T("merge_q") + "\"" + dataGridView1["Nazwa", dataGridView1.CurrentRow.Index].Value.ToString() + "\"" + _T("with") + "\"" + dataGridView1["Nazwa", Row].Value.ToString() + "\"?", _T("data_merge"), MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                        {
                            ...
                            MergeCommand.CommandText = "UPDATE " + this.SelectedTable + " " +
                                                        "SET " +
                                                            "id_wydawcy = (CASE WHEN id_wydawcy = ? THEN ? ELSE id_wydawcy END) " +
                                                            "WHERE id_wydawcy = ?; ";
                           /* MergeCommand.Parameters.AddWithValue("@IDOldPublisher", Int32.Parse(dataGridView1["ID wydawcy", Row].Value.ToString()));
                            MergeCommand.Parameters.AddWithValue("@IDNewPublisher", Int32.Parse(dataGridView1["ID wydawcy", dataGridView1.CurrentRow.Index].Value.ToString()));*/
                            MergeCommand.Parameters.AddWithValue("@IDOldPublisher", Int32.Parse(dataGridView1["ID wydawcy", dataGridView1.CurrentRow.Index].Value.ToString()));
                            MergeCommand.Parameters.AddWithValue("@IDNewPublisher", Int32.Parse(dataGridView1["ID wydawcy", Row].Value.ToString()));
                            MergeCommand.Parameters.AddWithValue("@IDOldPublisher", Int32.Parse(dataGridView1["ID wydawcy", dataGridView1.CurrentRow.Index].Value.ToString()));

                            MergeCommand.ExecuteNonQuery();

                            //DeletePublisher(Int32.Parse(dataGridView1["ID wydawcy", Row].Value.ToString()));

                            DeletePublisher(Int32.Parse(dataGridView1["ID wydawcy", dataGridView1.CurrentRow.Index].Value.ToString()));
        ...
        private void DelButton_Click(object sender, EventArgs e)
        {
            ...
            if (dataGridView1.Rows.Count > 0)
            {
                ...
                {
                    if (MessageBox.Show(_T("show_res_q") + " " + dataGridView1["Nazwa", dataGridView1.CurrentRow.Index].Value.ToString() + "?", "", MessageBoxButtons.YesNo) == System.Windows.Forms.DialogResult.Yes)
                    {
                        ShowList ShowListForm = new ShowList(GetListWherePublisherOccured(Int32.Parse(dataGridView1["Id wydawcy", dataGridView1.CurrentRow.Index].Value.ToString())), dataGridView1["Nazwa", dataGridView1.CurrentRow.Index].Value.ToString());
                        ShowListForm.ShowDialog();
                    }
                }
                if (MessageBox.Show(_T("to_delete") + " " + dataGridView1["Nazwa", dataGridView1.CurrentRow.Index].Value.ToString() + "?", _T("question"), MessageBoxButtons.YesNo, MessageBoxIcon.Question) == System.Windows.Forms.DialogResult.Yes)
                {
                    //if (DeletePublisherFromMagazines(Int32.Parse(dataGridView1["ID wydawcy", dataGridView1.CurrentRow.Index].Value.ToString())))
                        DeletePublisher(Int32.Parse(dataGridView1["ID wydawcy", dataGridView1.CurrentRow.Index].Value.ToString()));
                }
            }
        }
        ...
        private void DeletePublisher(int IDPublisher)
        {
            try
            {
                ...
                DeleteCommand.CommandText = "ModifyPublisher '', '', '', '', '', '', 'FALSE', '', 1, ?, 'c', 3; ";
        ...
        private void MagazinesPublishers_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void PrintButton_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                ReportCommand.CommandText = "SELECT LTRIM(RTRIM(nazwa_w)) AS nazwa_w, LTRIM(RTRIM(sk_nazwa_w)) AS sk_nazwa_w, LTRIM(RTRIM(id_panst_w)) AS id_panst_w, LTRIM(RTRIM(miasto_w)) AS miasto_w, LTRIM(RTRIM(kontakt_w)) AS kontakt_w, LTRIM(RTRIM(adres_w)) AS adres_w FROM wydawca_c ORDER BY nazwa_w;";

                ShowReport Report = new ShowReport(ReportCommand, "MagazinesPublisherReport");
