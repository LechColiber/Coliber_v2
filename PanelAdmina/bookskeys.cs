...
{
    public partial class BooksKeys : Form
    {
        ...
        public BooksKeys(Connection NewConnection, string Caption, string SelectedTable)
        {
            InitializeComponent();
        ...
        private List<string> GetListWhereKeyOccured(int IDKey)
        {
            try
            {
                OdbcCommand Command = new OdbcCommand();
                Command.Connection = NewConnection.AppConnection;
                Command.CommandText = "SELECT LTRIM(RTRIM(tytul_gl)) AS tytul_gl " +
	                                    "FROM dbo.ksiazki_new " +
                                        "LEFT JOIN dbo.ksi_klu ON ksiazki_new.kod = ksi_klu.kod " +
		                                "LEFT JOIN dbo.klucze_k ON klucze_k.kody = ksi_klu.kody " +
                                        "WHERE klucze_k.kody = ? " +
		                                "ORDER BY tytul_gl; " ;
        ...
        private void AddButton_Click(object sender, EventArgs e)
        {
            ...
            {
                EditKeysForm EKF = new EditKeysForm(NewConnection, ReturnIDRodzaj(RodzajDataTable), 1, "", -1, 'k');
                EKF.ShowDialog();
                SelectFromDb(ReturnIDRodzaj(RodzajDataTable));
            }
        }
        ...
        private void DelButton_Click(object sender, EventArgs e)
        {
            ...
            if (dataGridView1.Rows.Count > 0)
            {
                ...
                if (MessageBox.Show(_T("to_delete") + " " + dataGridView1["Klucze", dataGridView1.CurrentRow.Index].Value.ToString() + "?", _T("db_deleting"), MessageBoxButtons.YesNo, MessageBoxIcon.Question) == System.Windows.Forms.DialogResult.Yes)
                {
                    if (DeleteKeyFromBooks())
                        DeleteKey(Int32.Parse(dataGridView1["Kody", dataGridView1.CurrentRow.Index].Value.ToString()));
                }
            }
        }
        ...
        private bool DeleteKeyFromBooks()
        {
            try
            {
                OdbcCommand DeleteCommand = new OdbcCommand();
                ...
                DeleteCommand.CommandText = "DELETE FROM dbo.ksi_klu WHERE kody = ?; ";
        ...
        private void PrintButton_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                ReportCommand.CommandText = "SELECT LTRIM(RTRIM(klucze)) AS [klucze] FROM dbo.klucze_k ORDER BY klucze; ";

                ShowReport Report = new ShowReport(ReportCommand, "BooksKeysReport");
        ...
        private void BooksKeys_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void EditButton_Click(object sender, EventArgs e)
        {
            ...
            if (dataGridView1.CurrentRow != null)
            {
                ...
                EditKeysForm EKF = new EditKeysForm(NewConnection, Int32.Parse(dataGridView1.CurrentRow.Cells["id_rodzaj"].Value.ToString()), 2, dataGridView1.CurrentRow.Cells["Klucze"].Value.ToString(), Int32.Parse(dataGridView1.CurrentRow.Cells["Kody"].Value.ToString()), 'k');
                EKF.ShowDialog();
