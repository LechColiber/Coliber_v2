...
<script type="text/javascript" language="javascript">
	$(document).ready(function () {
		...
		function BindComobobox() {
			...
			if ($('#<%=dropPath.ClientID %>').val() == "books" && !$('#BasicSearchRadio').is(":checked")) {
				$('#SortTR').show("fast");
			}
			else{
				$('#SortTR').hide("fast");
		...
		function SearchInDb(CurrentPage, NewSearch, ValuesArray, ValuesArray2, Sort, Basic) {
			...
			function OnSuccess(data) {
				...
				var ShowOrderButtons = '<%=  (!string.IsNullOrEmpty(Session["CanOrder"].ToString()) != null ? Boolean.Parse(Session["CanOrder"].ToString()) : false) && dropPath.SelectedValue == "books" %>' && $('#<%=dropPath.ClientID %>').val() == 'books';
		...
		$(document).on("click", "#title_thead a", function () {
			Sort = 'title';
			SearchInDb(1, true, ValuesArray, ValuesArray2, 'title', Basic);
			$("#title_thead").append('\t' + '&#8595;');            
		});

		$(document).on("click", "#author_thead a", function () {
			Sort = 'author';
			SearchInDb(1, true, ValuesArray, ValuesArray2, 'author', Basic);
			$("#author_thead").append('\t' + '&#8595;');            
		});

		$(document).on("click", "#publish_year_thead a", function () {
			Sort = 'year_asc';
			SearchInDb(1, true, ValuesArray, ValuesArray2, 'year_asc', Basic);
			$("#publish_year_thead").append('\t' + '&#8595;');            
		});
