    ...
    class ReportInfo
    {
        ...
        public DataTable GetData(string dataSetName, ReportParameterInfoCollection reportParams)
        {
            ...
            if (dataSetInfo.dataTable == null)
            {
                ...
                {
                    ConnectionDialog dialog = new ConnectionDialog(dataSetInfo.dataSourceName, dataSourceInfo.connectString);
                ...
                if (Settings.CurrentType == MainForm.TypeEnum.Archivarius)
                {
                    try
                    {
                        XmlDocument XMLDocConfig = new XmlDocument();
                        XMLDocConfig.Load(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData) +
                                          "\\Archivarius - Lokale\\config.xml");

                        XmlNode ServerSetting = XMLDocConfig.SelectSingleNode("/configuration/connectionstring");
                        dataSourceInfo.connection = new SqlConnection(ServerSetting.InnerText);
                    }
                    catch (Exception Ex)
                    {
                ...
                else if (Settings.CurrentType == MainForm.TypeEnum.Coliber)
                {
                    try
                    {
                        dataSourceInfo.connection = new SqlConnection(Settings.ConnectionString);
                    }
                    catch (Exception Ex)
                    {
                        MessageBox.Show(Ex.Message);
                ...
                /*OdbcConnectionStringBuilder a = new OdbcConnectionStringBuilder(@"Driver=SQL SERVER;SERVER=LAPTOP\SQLEXPRESS;DATABASE=coliber_b1;Trusted_Connection=Yes");
                object Database;
                object Server;
                a.TryGetValue("database", out Database);
                a.TryGetValue("server", out Server);*/


                //dataSourceInfo.connection = new SqlConnection("Server=" + Server.ToString() + "; Database=" + Database.ToString() + ";Integrated Security = true;");

                dataSourceInfo.connection.Open();
                ...
                if (dataSetInfo.queryParameters != null)
                {
                    foreach (QueryParameterInfo queryParameterInfo in dataSetInfo.queryParameters)
                    {
                        ...
                        reportParameterValue = reportParameterValue.Replace(" 00:00:00", "");
                        lOK = DateTime.TryParseExact(reportParameterValue, "dd-MM-yyyy", CultureInfo.InvariantCulture, DateTimeStyles.None, out parsedDateTime);
                        if (lOK)
                        {
                            reportParameterValue = parsedDateTime.ToString("yyyy-MM-dd");
                        }
