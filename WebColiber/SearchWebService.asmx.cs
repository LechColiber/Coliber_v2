...
namespace WebColiber
{
	...
	public class SearchWebService : System.Web.Services.WebService
	{
		...
		{            
            ...
            if (Type == "novelties")
            {
                int iStop = StartIndex + Count;
                string cOd, cDo;
                cOd = Values[0];
                cDo = Values[1];
                cOd = cOd.Substring(6, 4) + cOd.Substring(3, 2) + cOd.Substring(0, 2);
                cDo = cDo.Substring(6, 4) + cDo.Substring(3, 2) + cDo.Substring(0, 2);
                Command.CommandText =
                    ...
                    ");\n" +
                     ";with rok_zap AS(\n" +
                        "SELECT kod FROM sygnat WHERE (data_zap >= '" + cOd + "' and data_zap <= '" + cDo + "'))\n" +
                    "SELECT kod INTO #rok_zap0 FROM rok_zap \n" +
                     ";with result AS\n" +
                    "(\n" +
                    "SELECT DISTINCT ksiazki_new.kod, syg_expand.syg_expand FROM ksiazki_new\n" +
                    "OUTER APPLY (SELECT TOP 1 syg_expand FROM sygnat WHERE sygnat.kod = ksiazki_new.kod) syg_expand WHERE ( ksiazki_new.kod in (select kod from #rok_zap0)) \n" +
                    ") \n" +
                    " INSERT INTO @ResultOrder \n" +
                    " SELECT ksiazki_new.kod, '' \n" +
                    " FROM ksiazki_new \n" +
                    " INNER JOIN result ON result.kod = ksiazki_new.kod \n" +
                    " ORDER BY ksiazki_new.tytul_gl; \n" +
                    " SELECT OrderID, ResultID AS kod, LTRIM(RTRIM(ksiazki_new.tytul_gl)) AS tytul, ksiazki_new.rok_wyd AS rok_wydania, 'Book' AS type,  CASE WHEN EXISTS (SELECT 1 FROM sygnat WHERE kod = ResultID) THEN (SELECT COUNT(*) FROM sygnat WHERE sygnat.kod = ResultID AND wypozycz = 0)ELSE -1 END AS ilosc, (SELECT COUNT(*) FROM @ResultOrder) AS rows_count, \n" +
                    " ('<ul>' + ISNULL((SELECT stuff((select LTRIM(RTRIM(nazwisko)) + ' ' + LTRIM(RTRIM(imie)) FROM list_aut_k INNER JOIN ksiazki_autor_new ON ksiazki_autor_new.id_autor = list_aut_k.id_aut WHERE ksiazki_autor_new.id_ksiazka = ksiazki_new.kod ORDER BY rating for xml path('li')),1,0,'')), '') + ISNULL((SELECT stuff((select LTRIM(RTRIM(nazwisko)) + ' ' + LTRIM(RTRIM(imie)) FROM list_aut_k INNER JOIN ksiazki_wautor_new ON ksiazki_wautor_new.id_wautor = list_aut_k.id_aut WHERE ksiazki_wautor_new.id_ksiazka = ksiazki_new.kod ORDER BY rating for xml path('li')),1,0,'')), '') + '</ul>') as autor \n" +
                    " FROM @ResultOrder \n" +
                    " INNER JOIN ksiazki_new ON ksiazki_new.kod = ResultID \n" +
                    " WHERE OrderID >= " + StartIndex.ToString() + " AND OrderID < " + iStop.ToString() + " ORDER BY OrderID; \n" +
                    "drop table #rok_zap0";
            } 
            else
            {
			...
			switch (Type)
			{
				...
				case "magazines": Command = getMagazinesQuery(Lista, ANDLista, StartIndex, Count, Sort); break;
                case "normy": Command = getNormyQuery(Lista, ANDLista, StartIndex, Count, Sort); break;
            }

            WriteLog(Command.CommandText,true);

            StringBuilder Output = new StringBuilder();
			StringBuilder Temp = new StringBuilder();

			DataTable Dt = CommonFunctions.ReadData(Command, ref Settings.ColiberConnection);
            if (Type == "novelties") Type = "books";
			if (Dt.Rows.Count == 0)
			{
			...
			{
                Output.Append("<div id='SearchType' style=\"visibility:hidden\">" + Type + "</div>");
                ...
                if (Type == "normy")
                {
                    Output.Append("<th align=\"center\" id='nrn_thead' style='padding-left: 10px; padding-right: 10px;'><a href='#'>  Numer normy  </a></th>");
                }
                ...
                if (Type == "normy")
                {
                    Output.Append("<th align=\"center\" id='data_pub_thead' style='padding-left: 10px; padding-right: 10px; width:130px'><a href='#'>Data publikacji</a></th>");
                }
				...
				for (int i = 0; i < Dt.Rows.Count; i++)
				{
					...
					if (Dt.Rows[i]["ilosc"].ToString() == "-1" || (Dt.Rows[i]["ilosc"].ToString() == "0" && Type == "normy"))
                    ...
                    if (Type == "normy")
                    {
                        Temp.Append("<td align=\"center\">" + Dt.Rows[i]["nr_normy"].ToString() + "</td>");
                    }
                    ...
                    if (Type == "normy")
                    {
                        Temp.Append("<td align=\"center\">" + CommonFunctions.DTOC(Dt.Rows[i]["rok_wydania"]) + "</td>");
                    }
		...
		public SqlCommand getBooksQuery(List<string[]> Values, List<string[]> ANDValues, int StartIndex, int Count, string Sort)
		{
			...
			{
                case "syg_desc":    Command.CommandText += " ORDER BY result.syg_expand desc;" + Environment.NewLine; break;
                ...
                case "author_desc":
                    Command.CommandText += " OUTER APPLY (SELECT TOP 1 imie_nazwisko AS nazwisko FROM list_aut_k " + Environment.NewLine +
                                            ...
                                            "ORDER BY CASE WHEN nazwisko IS NULL THEN 1 ELSE 0 END, nazwisko desc, ksiazki_new.tytul_gl; " + Environment.NewLine;
                    break;
                case "author":
                    Command.CommandText += " OUTER APPLY (SELECT TOP 1 imie_nazwisko AS nazwisko FROM list_aut_k " + Environment.NewLine +
                                            "   INNER JOIN ksiazki_autor_new on list_aut_k.id_aut = ksiazki_autor_new.id_autor" + Environment.NewLine +
                    ...
                    break;
                case "title_desc":
                    Command.CommandText += " ORDER BY Replace(Replace(ksiazki_new.tytul_gl,'[',''),']','') desc;" + Environment.NewLine; break;
                case "title":
				default:            Command.CommandText += " ORDER BY Replace(Replace(ksiazki_new.tytul_gl,'[',''),']','');" + Environment.NewLine; break;
			}
		...
		public static SqlCommand getMagazinesQuery(List<string[]> Values, List<string[]> ANDValues, int StartIndex, int Count, String Sort)
		{
            ...
            {
                case "title_desc":
                    Command.CommandText += " ORDER BY Replace(Replace(tytul,'[',''),']','') desc;" + Environment.NewLine; break;
                case "title":
                default: Command.CommandText += " ORDER BY Replace(Replace(tytul,'[',''),']','');" + Environment.NewLine; break;
            }

            Command.CommandText += " SELECT OrderID, tytul, ResultID AS kod, 'Magazine' AS type, 0 AS ilosc, (SELECT COUNT(*) FROM @ResultOrder) AS rows_count" + Environment.NewLine +
								   " FROM @ResultOrder" + Environment.NewLine +
								   " WHERE OrderID >= @start AND OrderID < @end ORDER BY OrderID; " + Environment.NewLine;
        ...
        public static SqlCommand getNormyQuery(List<string[]> Values, List<string[]> ANDValues, int StartIndex, int Count, string Sort)
        {
            ...
            string SearchCommand = " ;with result AS" + Environment.NewLine +
                                   "(" + Environment.NewLine +
                                   "SELECT DISTINCT Normy.id_norma FROM dbo.Normy" + Environment.NewLine;
            ...
            for (int i = 0; i < Values.Count; i++)
            {
                ...
                {
                    case "nrn":
                        Command.CommandText += ";with nrn_tab AS (SELECT id_norma FROM Normy WHERE nr_normy like @fraza" + i + ")" + Environment.NewLine +
                                               "SELECT id_norma INTO #nrn_tab" + i + " FROM nrn_tab;" + Environment.NewLine;
                        Command.Parameters.AddWithValue("@fraza" + i, Phrase.Replace("\"","") + "%");
                        SearchCommand += LewyNawias + " Normy.id_norma in (select id_norma from #nrn_tab" + i + ")" + PrawyNawias + Environment.NewLine;
                        dropTables += "drop table #nrn_tab" + i + "; ";
                        break;
                    case "title":
                        Phrase = string.Join(" AND", Phrase.Replace(".", " ").Split(' ').Where(x => x.Trim().Length > 1).Select(x => "\"" + x.Trim() + "\""));
                        if (Phrase.Trim().Length == 0) Phrase = "\"" + temp[0].Trim() + "*\"";
                        Command.CommandText += ";with tytul_tab AS(SELECT id_norma FROM Normy WHERE Contains(tytul, @fraza" + i + "))" + Environment.NewLine +
                                               "SELECT id_norma INTO #tytul_tab" + i + " FROM tytul_tab;" + Environment.NewLine;
                        Command.Parameters.AddWithValue("@fraza" + i, Phrase );
                        SearchCommand += LewyNawias + " Normy.id_norma in (select id_norma from #tytul_tab" + i + ")" + PrawyNawias + Environment.NewLine;
                        dropTables += "drop table #tytul_tab" + i + "; ";
                        break;
                    case "statusn":
                            Command.CommandText += ";with status_tab AS(SELECT id_norma FROM Normy WHERE StatusN = @fraza" + i + ")" + Environment.NewLine +
                                                   "SELECT id_norma INTO #status_tab" + i + " FROM status_tab;" + Environment.NewLine;
                            Command.Parameters.AddWithValue("@fraza" + i, Phrase);
                            SearchCommand += LewyNawias + " Normy.id_norma in (select id_norma from #status_tab" + i + ")" + PrawyNawias + Environment.NewLine;
                            dropTables += "drop table #status_tab" + i + "; ";
                        break;
                    case "data_pub":
                        if (DateTime.TryParseExact(Phrase, System.Globalization.CultureInfo.CurrentCulture.DateTimeFormat.ShortDatePattern, null, DateTimeStyles.None, out tTemp)){
                            Command.CommandText += ";with data_pub_tab AS(SELECT id_norma FROM Normy WHERE data_pub like @fraza" + i + ")" + Environment.NewLine +
                                               "SELECT id_norma INTO #data_pub_tab" + i + " FROM data_pub_tab;" + Environment.NewLine;
                            Phrase = tTemp.ToString("yyyy-MM-dd");
                            WriteLog(Phrase);
                            Command.Parameters.AddWithValue("@fraza" + i, Phrase);
                            SearchCommand += LewyNawias + " Normy.id_norma in (select id_norma from #data_pub_tab" + i + ")" + PrawyNawias + Environment.NewLine;
                            dropTables += "drop table #data_pub_tab" + i + "; ";
                        }
                        else lSkipOr = true;
                        break;
                    case "key":
                        Command.CommandText += ";with klucze_tab AS(SELECT DISTINCT nor_nor_id as id_norma FROM normy_klucz WHERE nazwa like @fraza" + i + ")" + Environment.NewLine +
                                               "SELECT id_norma INTO #klucze_tab" + i + " FROM klucze_tab;";
                        Command.Parameters.AddWithValue("@fraza" + i, Phrase + "%");
                        SearchCommand += LewyNawias + " normy.id_norma in (select id_norma from #klucze_tab" + i + ")" + PrawyNawias + Environment.NewLine;
                        dropTables += "drop table #klucze_tab" + i + "; ";
                        break;
                }

                if (i + 1 < Values.Count && lSkipOr == false)
                    SearchCommand += Operator.ToLower() == "and" ? " AND " : " OR ";
            }
            ...
            for (int i = 0; i < ANDValues.Count; i++)
            {
                ...
                {
                    case "lang":
                        SearchCommand += "(SELECT jezyk FROM jezyki WHERE id = @fraza_" + i + ") IN (jezyk1, jezyk2, jezyk3)" + Environment.NewLine;
                        Command.Parameters.AddWithValue("@fraza_" + i, Phrase.ToUpper());
                        break;
                    case "country":
                        SearchCommand += "czasop.kraj_w = (SELECT panstwo FROM panstwa WHERE id = @fraza_" + i + ") " + Environment.NewLine;
                        Command.Parameters.AddWithValue("@fraza_" + i, Phrase.ToUpper());
                        break;
                    case "sources":
                        SearchCommand += "EXISTS (SELECT 1 FROM documents WHERE kod = Normy.id_norma AND rodzaj_zasobu = 4)" + Environment.NewLine;
                        break;
                }
            ...
            Command.CommandText += " INSERT INTO @ResultOrder" + Environment.NewLine +
                                   " SELECT Normy.id_norma, LTRIM(RTRIM(Normy.tytul)) AS tytul" + Environment.NewLine +
                                   " FROM Normy" + Environment.NewLine +
                                   " INNER JOIN result ON result.id_norma = Normy.id_norma" + Environment.NewLine;

            //WriteLog(Sort);
            switch (Sort.ToLower().Trim())
            {
                case "nr_normy": Command.CommandText += " ORDER BY Normy.nr_normy;" + Environment.NewLine; break;
                case "data_pub": Command.CommandText += " ORDER BY Normy.data_pub;" + Environment.NewLine; break;
                case "title":
                default: Command.CommandText += " ORDER BY Normy.Tytul;" + Environment.NewLine; break;
            }
            Command.CommandText += " SELECT OrderID, Normy.nr_normy,Normy.tytul,data_pub as rok_wydania, ResultID AS kod, 'Norma' AS type, (SELECT COUNT(*) FROM zasoby WHERE zasoby.kod = ResultID AND rodzaj_zas = 4 AND stan = 0) AS ilosc, (SELECT COUNT(*) FROM @ResultOrder) AS rows_count" + Environment.NewLine +
                                   " FROM @ResultOrder" + Environment.NewLine +
                                   " INNER JOIN normy ON normy.id_norma = ResultID \n" +
                                   " WHERE OrderID >= @start AND OrderID < @end order by OrderID " + Environment.NewLine;
